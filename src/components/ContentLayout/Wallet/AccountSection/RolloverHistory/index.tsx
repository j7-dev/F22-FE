...
import {
    fakeCouponHistory,
    CouponHistoryTitle,
} from '../../RewardSection/CouponHistory/fakeData';
            ...
            <div className="tableWrap overflow-x-scroll">
                <div className="tableHeader h-10 w-full items-center flex flex-row text-white bg-[#2B3240] md:pl-5 px-0 font-bold text-[13px] text-start">
                    {CouponTitle.map((item) => (
                        <div
                            className={`bg-[#2B3240] h-full flex items-center px-1.5 ${
                                window.innerWidth < 768
                                    ? 'whitespace-nowrap min-w-[115px] max-w-[115px]'
                                    : 'w-1/6 '
                            }`}
                        >
                            {' '}
                ...
                {CouponHistory.length > 0 ? (
                    CouponHistory.map((item) => (
                        <div
                            key={nanoid()}
                            className="tableContent h-10 w-full items-center flex flex-row  md:pl-5 px-0 font-bold text-[13px] text-start border-0 border-solid border-b border-[#F3F3F4]"
                        >
                            {CouponTitle.map((title) => (
                                <div
                                    key={nanoid()}
                                    className={`h-full flex items-center px-1.5 ${
                                        window.innerWidth < 768
                                            ? 'whitespace-nowrap min-w-[115px] max-w-[115px]'
                                            : 'w-1/6 '
                                    }`}
