    ...
    return (
        <div className="w-full bg-white rounded-lg shadow-[0_0px_29px_0px_rgba(43, 50, 64, 0.09)] flex flex-col gap-2.5 p-4">
            <div className="transactionButtonContainer flex gap-1 justify-between items-center">
                ...
                <div className={`${selectedSection === 'Deposit' ? 'active' : ''} Deposit transactionButton flex gap-1 justify-center items-center h-10  rounded-lg w-1/3`} onClick={() => handleClick('Deposit')}>
                    <div className="transactionIcon md:w-[30px] md:h-[30px] w-5 h-5" />
                    <span className="w-auto font-bold text-sm">{t('Deposit')}</span>
                </div>
                <div className={`${selectedSection === 'Withdraw' ? 'active' : ''} Withdraw transactionButton flex gap-1 justify-center items-center h-10  rounded-lg w-1/3`} onClick={() => handleClick('Withdraw')}>
                    <div className="transactionIcon md:w-[30px] md:h-[30px] w-5 h-5" />
