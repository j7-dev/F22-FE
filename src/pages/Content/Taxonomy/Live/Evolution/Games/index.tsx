...
const Games: React.FC<GamesDataProp> = ({ ProviderName, gamesData = [], isLoading }) => {
    ...
    //排除rng-類別
    const gamesList = GameTypeValue !== 'all' ? gamesData.filter((item) => casinoCategoryFilter[GameTypeValue].some((casinoCategory) => item['Game Type'] === casinoCategory) && !item['Game Type'].startsWith('rng-')) : gamesData.filter((item) => !item['Game Type'].startsWith('rng-'));
    return (
        <>
