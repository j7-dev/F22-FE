...
const index = () => {
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-2 sm:gap-4 gap-2 mb-8">
                {DEPOSIT_METHOD.map((item) => (
                    <div>
                        <Button key={item.value} onClick={handleClick(item.value)} className={`${isSelect === item.value ? 'bg-[#9680EA] text-white' : 'bg-[#F8F9FF] text-[#9680EA]'} col-span-1 w-full h-10 sm:h-[65px] sm:text-xl text-base font-bold border-[#9680EA] hover:text-white hover:bg-[#9680EA]`}>
                            {t(item.name)}
                        </Button>
                        {item.value === 'codePay' ? (
