        ...
        filters.map((filter) => {
            if (filter.operator !== 'or' && filter.operator !== 'and' && 'field' in filter) {
                ...
                if (Array.isArray(value)) {
                    value.map((val, index) => {
                        rawQuery += `&filters${generateNestedFilterField(field)}[$${mapedOperator}][${index}]=${val}`;
                    });
                } else {
                    rawQuery += `&filters${generateNestedFilterField(field)}[$${mapedOperator}]=${value}`;
                }
            } else {
                const { value } = filter;

                value.map((item, index) => {
                    const { field, operator, value: v } = item as LogicalFilter;

                    const mapedOperator = mapOperator(operator);

                    rawQuery += `&filters[$${filter.operator}][${index}]${generateNestedFilterField(field)}[$${mapedOperator}]=${v}`;
                });
            }
        });
    }
